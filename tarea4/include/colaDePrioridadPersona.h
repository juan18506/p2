/*
  ...
  primer evento en la agenda es menor que la fecha del primer evento de la otra
  persona. Este criterio se puede modificar, haciendo que la persona prioritaria
  sea la de evento con fecha mayor.
...
/*
  ...
  La prioridad entre dos personas se define por la que tenga fecha menor en el
  primer evento de la agenda. El tiempo de ejecución en el peor caso es
  Theta(N).
 */
TColaDePrioridadPersona crearCP(nat N);

/*
  Modifica el criterio de prioridad: si el prioritario era la persona que tenía
  asociada la fecha menor pasa a ser la que tiene asociada la fecha mayor, y
  viceversa. Debe modificar la cola de forma de que se respete el nuevo criterio
  de prioridad. Se pide que el tiempo de ejecución en el peor caso sea O(n*log
  n), siendo 'n' la cantidad de elementos de 'cp'. Sin embargo, existe una
  solución que lo hace en O(n).
 */
void invertirPrioridad(TColaDePrioridadPersona &cp);

/*
  Libera la memoria asignada a 'cp' y a cada uno de sus elementos.
  Es decir, libera la memoria asociada a todas las personas que estén en la
  cola. El tiempo de ejecución en el peor caso es O(N*m), siendo 'N' el
  parámetro pasado en crearCP y 'm' la cantidad de eventos en la agenda con
...
/*
  ...
  El tiempo de ejecución en el peor caso es Theta(log N) donde 'N' el parámetro
  pasado en crearCP. PRE: !estaEnCP(persona, cp) PRE: 1 <= idPersona <= N,
  siendo 'N' el parámetro pasado en crearCP.
 */
...
/*
  ...
  El tiempo de ejecución en el peor caso es O(log N + m), siendo 'N' el
  parámetro pasado en crearCP y 'm' la cantidad de eventos en la agenda de la
  persona prioritaria.
 */
